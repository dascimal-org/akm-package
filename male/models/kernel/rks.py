...
class RKS(KSGD):
    ...
    def _init_params(self, x):
        ...
        if self.scale_learning_rate > 0:
            self.learning_rate = self.learning_rate / x.shape[0]
    ...
    def predict_from_wx(self, wx):
        if self.num_classes == 2:
            y = self._decode_labels(np.uint8(wx >= 0))
        else:
            y = self._decode_labels(np.argmax(wx, axis=1))
        return y
