...
def run_one_candidate(
        ...
        cross, num_features, keep_vars, ind_test=None, grid_search=True):
    ...
    for ri in range(total_runs):
        ...
        if grid_search:
            print('Acc on valid set: {}, Acc on training set: {}, Training time: {} seconds'
                  .format(test_acc, train_acc, int(train_time)))
        else:
            print('Acc on testing set: {}, Acc on training set: {}, Training time: {} seconds'
                  .format(test_acc, train_acc, int(train_time)))
...
def log_result(result):
    ...
    if grid_search:
        print('Acc on valid set: {}, Acc on training set: {}, Training time: {} seconds, Params: {}'.
              format(test_acc, train_acc, int(time), dict2string(params)))
    else:
        print("\n========== FINAL RESULT ==========")
        print('Data set: {}'.format(log_lst[len(log_lst) - 1]['dataset']))
        print('Acc on testing set: {}, Acc on training set: {}, Training time: {} seconds'
              .format(test_acc, train_acc, int(time)))
...
def run_grid_search_multicore(
        ...
        num_runs=3, cross=0, num_features=None, full_dataset=None, keep_vars=[], ind_test=None):
    if dataset is None:
        if len(sys.argv) > 2:
            dataset = sys.argv[2]
        else:
    ...
    for candidate_params in candidate_params_lst:
        result = pool.apply_async(
            ...
            args=(
                ...
                num_features, keep_vars, ind_test, grid_search),
            callback=log_result
        )
    ...
    if len(candidate_params_lst) > 1:
        ...
        print('Best acc on training set: {}'.format(train_acc_lst[idx_best]))
